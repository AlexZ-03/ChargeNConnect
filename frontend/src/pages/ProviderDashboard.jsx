...
const ProviderDashboard = () => {
  ...
  const handleStationSubmit = async (stationData) => {
    try {
      ...
      const response = await axios[method](url, stationData, {
        headers: {
      ...
      const errorMessage = error.response?.data?.message || 'Failed to process station';
        ...
        {activeTab === 1 && (
          <>
            ...
            {loading ? (
              ...
              <Alert severity="info">No stations found. Add your first station!</Alert>
            ) : (
                        ...
                        <Typography color={theme.text.secondary}>
                          Location: {
                            typeof station.location === 'object' 
                              ? (station.location.address || 'No address specified')
                              : (station.location || 'No location specified')
                          }
                        </Typography>
                        <Typography color={theme.text.primary} sx={{ mt: 2 }}>
                        ₹{station.pricePerHour?.toFixed(2) || '0.00'}/hour
                        </Typography>
                        <Typography 
                          sx={{ 
                    ...
                    <Card sx={{ 
                      ...
                      <CardContent>
                        ...
                        <Typography color={theme.text.secondary}>
                          Location: {
                            booking.station?.location ? (
                              typeof booking.station.location === 'object'
                                ? (booking.station.location.address || 'No address specified')
                                : booking.station.location
                        ...
                        <Typography color={theme.text.primary}>
                          Price: ₹{booking.totalPrice?.toFixed(2) || '0.00'}
                        </Typography>
                        <Typography 
                          sx={{ 
